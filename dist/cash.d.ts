...
}, indexOf: (searchElement: any, fromIndex?: number) => number, map: <U>(callbackfn: (value: any, index: number, array: any[]) => U, thisArg?: any) => U[], push: (...items: any[]) => number, slice: (start?: number, end?: number) => any[], some: (predicate: (value: any, index: number, array: any[]) => unknown, thisArg?: any) => boolean, splice: {
...
declare function isPlainObject(x: any): x is PlainObject<any>;
interface Cash {
    get(): EleLoose[];
...
declare function each<T, U extends ArrayLike<T> = ArrayLike<T>>(arr: U, callback: EachArrayCallback<T>, _reverse?: boolean): U;
declare function each<T, U extends PlainObject<T> = PlainObject<T>>(obj: U, callback: EachObjectCallback<T>): U;
interface Cash {
...
declare function extend(...sources: any[]): any;
